...
void WorldSession::HandleSplitItemOpcode(WorldPackets::Item::SplitItem const& packet)
{
    ...
    if (!pActor->IsValidPos(packet.srcbag, packet.srcslot, true))
    {
        _player->SendEquipError(EQUIP_ERR_ITEM_NOT_FOUND, nullptr, nullptr);
        return;
    }

    if (!pActor->IsValidPos(packet.dstbag, packet.dstslot, false))       // can be autostore pos
    {
        _player->SendEquipError(EQUIP_ERR_ITEM_DOESNT_GO_TO_SLOT, nullptr, nullptr);
        return;
    }

    pActor->SplitItem(src, dst, packet.count);
    if (suiActing)
...
void WorldSession::HandleSwapInvItemOpcode(WorldPackets::Item::SwapInvItem const& packet)
{
    ...
    if (!pActor->IsValidPos(INVENTORY_SLOT_BAG_0, packet.srcslot, true))
    {
        _player->SendEquipError(EQUIP_ERR_ITEM_NOT_FOUND, nullptr, nullptr);
        return;
    }

    if (!pActor->IsValidPos(INVENTORY_SLOT_BAG_0, packet.dstslot, true))
    {
        _player->SendEquipError(EQUIP_ERR_ITEM_DOESNT_GO_TO_SLOT, nullptr, nullptr);
        return;
    }

    if ((pActor->IsBankPos(INVENTORY_SLOT_BAG_0, packet.srcslot) || pActor->IsBankPos(INVENTORY_SLOT_BAG_0, packet.dstslot)) && !pActor->CanUseBank())
    {
...
void WorldSession::HandleSwapItem(WorldPackets::Item::SwapItem const& packet)
{
    ...
    if (!pActor->IsValidPos(packet.dstbag, packet.dstslot, true))
    {
        _player->SendEquipError(EQUIP_ERR_ITEM_DOESNT_GO_TO_SLOT, nullptr, nullptr);
        return;
    }

    if ((pActor->IsBankPos(packet.srcbag, packet.srcslot) || pActor->IsBankPos(packet.dstbag, packet.dstslot)) && !pActor->CanUseBank())
    {
        _player->SendEquipError(EQUIP_ERR_TOO_FAR_AWAY_FROM_BANK, nullptr, nullptr);
        ProcessAnticheatAction("ItemsCheck", "Attempt to cheat-bank items", CHEAT_ACTION_REPORT_GMS);
        return;
...
void WorldSession::HandleAutoEquipItemOpcode(WorldPackets::Item::AutoEquipItem const& packet)
{
    ...
    Item *pSrcItem  = pActor->GetItemByPos(packet.srcbag, packet.srcslot);
    if (!pSrcItem)
        return;                                             // only at cheat

    uint16 dest;
    InventoryResult msg = pActor->CanEquipItem(NULL_SLOT, dest, pSrcItem, !pSrcItem->IsBag());
    if (msg != EQUIP_ERR_OK)
    {
    ...
    Item *pDstItem = pActor->GetItemByPos(dest);
    if (!pDstItem)                                          // empty slot, simple case
    {
        pActor->RemoveItem(packet.srcbag, packet.srcslot, true);
        pActor->EquipItem(dest, pSrcItem, true);
        pActor->AutoUnequipOffhandIfNeed();
    }
    else                                                    // have currently equipped item, not simple case
    {
        ...
        msg = pActor->CanUnequipItem(dest, !pSrcItem->IsBag());
        if (msg != EQUIP_ERR_OK)
        {
        ...
        uint16 eSrc = 0;
        if (pActor->IsInventoryPos(src))
        {
            msg = pActor->CanStoreItem(packet.srcbag, packet.srcslot, sSrc, pDstItem, true);
            if (msg != EQUIP_ERR_OK)
                msg = pActor->CanStoreItem(packet.srcbag, NULL_SLOT, sSrc, pDstItem, true);
            if (msg != EQUIP_ERR_OK)
                msg = pActor->CanStoreItem(NULL_BAG, NULL_SLOT, sSrc, pDstItem, true);
        }
        else if (pActor->IsBankPos(src))
        {
            msg = pActor->CanBankItem(packet.srcbag, packet.srcslot, sSrc, pDstItem, true);
            if (msg != EQUIP_ERR_OK)
                msg = pActor->CanBankItem(packet.srcbag, NULL_SLOT, sSrc, pDstItem, true);
            if (msg != EQUIP_ERR_OK)
                msg = pActor->CanBankItem(NULL_BAG, NULL_SLOT, sSrc, pDstItem, true);
        }
        else if (pActor->IsEquipmentPos(src))
        {
            msg = pActor->CanEquipItem(packet.srcslot, eSrc, pDstItem, true);
            if (msg == EQUIP_ERR_OK)
                msg = pActor->CanUnequipItem(eSrc, true);
        }
        ...
        // now do moves, remove...
        pActor->RemoveItem(dstbag, dstslot, false);
        pActor->RemoveItem(packet.srcbag, packet.srcslot, false);
        ...
        // add to src
        if (pActor->IsInventoryPos(src))
            pActor->StoreItem(sSrc, pDstItem, true);
        else if (pActor->IsBankPos(src))
            pActor->BankItem(sSrc, pDstItem, true);
        else if (pActor->IsEquipmentPos(src))
            pActor->EquipItem(eSrc, pDstItem, true);
...
void WorldSession::HandleSellItemOpcode(WorldPackets::Item::SellItem const& packet)
{
    ...
    if (!packet.itemGuid || !pActor->IsInWorld())
        return;

    Creature* pCreature = pActor->GetNPCIfCanInteractWith(packet.vendorGuid, UNIT_NPC_FLAG_VENDOR);
    if (!pCreature)
    {
    ...
    Item *pItem = pActor->GetItemByGuid(packet.itemGuid);

    if (!pItem)
        return;

    // prevent sell not owner item
    if (pActor->GetObjectGuid() != pItem->GetOwnerGuid())
    {
    ...
    if (pActor->IsBankPos(pItem->GetPos()))
    {
        _player->SendSellError(SELL_ERR_CANT_SELL_ITEM, pCreature, packet.itemGuid, 0);
        return;
    }
    ...
    if (pActor->GetLootGuid() == pItem->GetObjectGuid())
    {
        _player->SendSellError(SELL_ERR_CANT_SELL_ITEM, pCreature, packet.itemGuid, 0);
        return;
    }
    ...
    {
        Item *pNewItem = pItem->CloneItem(actualCount, pActor);
        if (!pNewItem)
        {
        ...
        pActor->ItemRemovedQuestCheck(pItem->GetEntry(), actualCount);
        if (pActor->IsInWorld())
            pItem->SendCreateUpdateToPlayer(pActor);
        pItem->SetState(ITEM_CHANGED, pActor);

        pActor->AddItemToBuyBackSlot(pNewItem, money, packet.vendorGuid);
        if (pActor->IsInWorld())
            pNewItem->SendCreateUpdateToPlayer(pActor);
    }
    else
    {
        pActor->ItemRemovedQuestCheck(pItem->GetEntry(), pItem->GetCount());
        pActor->RemoveItem(pItem->GetBagSlot(), pItem->GetSlot(), true);
        pActor->InterruptSpellsWithCastItem(pItem);
        pItem->RemoveFromUpdateQueueOf(pActor);
        pActor->AddItemToBuyBackSlot(pItem, money, packet.vendorGuid);
    }

    pActor->LogModifyMoney(money, "SellItem", pCreature->GetObjectGuid(), pItem->GetEntry());
...
void WorldSession::HandleBuybackItem(WorldPackets::Item::BuybackItem const& packet)
{
    ...
    // [SUI] P4b: buy the item back into the DRIVEN bot's bags from its own buyback
    // slots and purse. Ops act on GetSuiActor(); errors stay on _player (commander);
    // re-snapshot after so the bot's bags and coin refresh.
    Player* pActor = GetSuiActor();
    bool suiActing = pActor != _player;
    ...
    Item *pItem = pActor->GetItemFromBuyBackSlot(slot);
    if (!pItem)
    {
    ...
    uint32 price = pActor->GetBuyBackItemPrice(slot);
    ...
    pActor->ModifyMoney(-static_cast<int32>(price));
    pActor->RemoveItemFromBuyBackSlot(slot, false);
    pActor->ItemAddedQuestCheck(pItem->GetEntry(), pItem->GetCount());
...
void WorldSession::HandleBuyItemInSlotOpcode(WorldPackets::Item::BuyItemInSlot const& packet)
{
    ...
    if (packet.bagGuid == pActor->GetObjectGuid())
        bag = INVENTORY_SLOT_BAG_0;
    else
    {
        for (int i = INVENTORY_SLOT_BAG_START; i < INVENTORY_SLOT_BAG_END; ++i)
        {
            if (Bag *pBag = (Bag*)pActor->GetItemByPos(INVENTORY_SLOT_BAG_0, i))
            {
    ...
    pActor->BuyItemFromVendor(packet.vendorGuid, packet.item, packet.count, bag, packet.bagslot);
    if (pActor != _player)
        SuiPossess::ResnapshotControlled(this);
}
...
void WorldSession::SendListInventory(ObjectGuid vendorguid, uint8 menu_type)
{
    ...
    // (→ MirrorOwnerPacket) when from a gossip "browse goods" option. The
    // not-found error stays on the commander so it is seen directly.
    Player* pActor = GetSuiActor();
    Creature* pCreature = pActor->GetNPCIfCanInteractWith(vendorguid, UNIT_NPC_FLAG_VENDOR);
    ...
    float discountMod = pActor->GetReputationPriceDiscount(pCreature);
    ...
    for (int i = 0; i < numitems; ++i)
    {
        ...
        if (crItem)
        {
            ...
            {
                if (!pActor->IsGameMaster())
                {
                    // class wrong item skip only for bindable case
                    if ((pProto->AllowableClass & pActor->GetClassMask()) == 0 && pProto->Bonding == BIND_WHEN_PICKED_UP)
                        continue;

                    // race wrong item skip always
                    if ((pProto->AllowableRace & pActor->GetRaceMask()) == 0)
                        continue;
                    ...
                    if (!pProto->RequiredReputationFaction && pProto->RequiredReputationRank > 0 &&
                        ReputationRank(pProto->RequiredReputationRank) > pActor->GetReputationRank(pCreature->GetFactionId()))
                        continue;
...
#if SUPPORTED_CLIENT_BUILD <= CLIENT_BUILD_1_6_1
                    if (pProto->RequiredReputationFaction && pProto->RequiredReputationRank > 0 &&
                        ReputationRank(pProto->RequiredReputationRank) > pActor->GetReputationRank(pProto->RequiredReputationFaction))
                        continue;
#endif

                    if (crItem->conditionId && !IsConditionSatisfied(crItem->conditionId, pActor, pCreature->GetMap(), pCreature, CONDITION_FROM_VENDOR))
                        continue;
                }
