...
// ============================================================================
// [SPREAD] CountNearbyBots — anti-convergence primitive (FINDING_005)
//
// Counts OTHER AI bots within `radius` of a center — the signal that a grind spot is a dogpile
// (8 bots on the Dragonmaw camp → 20-48-attacker overpulls). Excludes self and our own
// groupmates (a GROUP is SUPPOSED to stack + focus-fire; only INDEPENDENT solo bots converging is
// the problem). Real players never count. Used by the TASK_GRIND engage gate to defer a solo
// filler grind out of an over-subscribed camp so the no-kill breaker relocates us elsewhere.
// ============================================================================
uint32 AiBotAI::CountNearbyBots(Unit* pCenter, float radius) const
{
    ...
    Cell::VisitWorldObjects(pCenter, searcher, radius);

    Group* pMyGroup = me->GetGroup();
    uint32 count = 0;
        ...
        while (pRef)
        {
            if (Unit* pTarget = pRef->getSourceUnit())
            {
                    ...
                    me->IsWithinDist(pTarget, bestDist))
                {
                    ...
                    // objective (entry!=0, MatchesObjectiveEntry) is still honored, so a legit
                    // guard-kill quest is untouched. P1 has no level ceiling of its own, so a guard the
                    // town aggros onto the bot would otherwise be re-selected here past P2b's ceiling.
