                          ...
                          uint32 &dangerLevel)
{
    ...
    for (uint32 i = 0; i < (uint32)points.size(); i += STEP)
    {
        ...
        for (auto const& pair : allSpawns)
        {
            ...
            // case: "Pat's Snowcloud Guy" (15730, faction 114, L60, static_flags1=0x266) stands inside the
...
void AiBotAI::MoveToDestination(float destX, float destY, float destZ, bool stopCurrentMovement)
{
    ...
    // SHORTCUT / NOT_USING_PATH = the straight line PathFinder returns when it cannot lay a real
    // corridor -- flagged "travel through obstacles, terrain, air" -- which glides a ground bot
    // through the void toward the dest (the floating-toward-a-slope bug). A flying bot may use them;
    // a walking one must not, so refuse and hand off to the seam/ring/recovery machinery below.
    bool airLine = (pType & (PATHFIND_SHORTCUT | PATHFIND_NOT_USING_PATH)) && !me->CanFly();
    bool pathOk = !(pType & PATHFIND_NOPATH) && !airLine;
    ...
    if (!pathOk)
    {
        ...
        float ringDist = AIBOT_RING_SCAN_RADIUS;
        for (int i = 0; i < 8; ++i)
        {
    ...
    if (!pathOk)
    {
        ...
        // [FINDING_020] …except when the START is a disconnected island: the off-mesh re-tare only
    ...
    // [FINDING_017] A degenerate partial is not a route. Detour returns
    // INCOMPLETE partials TOWARD an unreachable dest, so the nudge/ring-scan
    // "not NOPATH" tests above can succeed with a leg that is ~0yd long —
    // it exhausts instantly and the leg-exhausted continuation re-issues at
    // log speed (measured: 1.07M zero-yd cycles / 15h fleet-wide). The
    // recovery chain is done by this point: report honest no_path so the C#
...
// ============================================================
// IsStartIsolated — [FINDING_020] "is it ME that can't path, or the dest?"
//
// The off-mesh re-tare at the top of MoveToDestination asks only "is there a navmesh poly under
// me?" — a bot on a DISCONNECTED island (the floor patch inside an inn/smithy WMO, the Menethil
// pier underside, the Crossroads graveyard bump, harbour water) HAS a poly, so it passes, and then
// every path it computes NOPATHs or comes back as a ≤5yd partial no matter the dest. Post-
// FINDING_011 (no straight-line shortcut) it also has no move that can get it off the island.
// The C# fleet no-path memory recorded those failures against the DEST and blacklisted reachable
// quest cells for the whole fleet (FINDING_020).
//
// Probe: can the bot path to ANY of 8 ring points AIBOT_ISOLATION_PROBE_YARDS out (ground-snapped
// so a hillside doesn't fail the end-poly lookup)? One real route (not NOPATH, not a degenerate
// partial) → the start is connected → false. All eight fail → isolated → true. Cost: ≤8 short
// calculates, and only on the MOVE_FAILED branches (≈1–2/s fleet-wide), never on the hot path.
// Conservative by construction: a false "isolated" only means C# keeps that one failure per-bot.
// ============================================================
// Probe radii. ROUND 2 (2026-08-21 08:45): 20yd was too small — the Menethil gate-tower pocket
// (53 bots, (-3655,-825)) is a ~25yd island, so every 20yd probe landed INSIDE it and passed;
// a 104-waypoint path over the bridge proved the town itself is connected. Two rings now, and a
// start is isolated only if it cannot reach ANY of the 16 points: the 60yd ring catches room/
// tower/pier/graveyard pockets, the 120yd ring keeps a bot deep in a connected cave or alley
// from being called isolated (its 120yd probe toward the mouth lands outside and paths).
#define AIBOT_ISOLATION_PROBE_YARDS   60.0f
#define AIBOT_ISOLATION_PROBE_YARDS_2 120.0f
bool AiBotAI::IsStartIsolated()
{
    float const sx = me->GetPositionX(), sy = me->GetPositionY(), sz = me->GetPositionZ();
    float const radii[2] = { AIBOT_ISOLATION_PROBE_YARDS, AIBOT_ISOLATION_PROBE_YARDS_2 };
    for (int r = 0; r < 2; ++r)
    for (int i = 0; i < 8; ++i)
    {
        float const ang = (float)i * (float)M_PI / 4.0f;
        float px = sx + cosf(ang) * radii[r];
        float py = sy + sinf(ang) * radii[r];
        float pz = sz;
        me->UpdateAllowedPositionZ(px, py, pz);   // ground the probe so slopes don't fail the end-poly lookup
        ...
        if (t & PATHFIND_INCOMPLETE)
        {
            ...
            // "partial with real length is a route" test passed for every pocket wider than 5yd —
            // 37 of 43 gate-pocket bots failed MOVE_TOs after the 08:48 rebuild and only 2 were
...
// ============================================================
// TeleportToWalkable — [FINDING_020 round 5] port to (x,y,z) and GUARANTEE the bot can walk away.
//
// The whole point of an escape port is to land somewhere the bot can path FROM. The requested coord
// is an inn / flight-path spot; if the exact pixel happens to be inside a wall or on a disconnected
// navmesh island, land there and the bot is stuck again. So: teleport to the request, check
// IsStartIsolated(), and if it's bad ring-search outward (8→16→32→64→120yd, 8 angles) for the first
// spot that is NOT isolated and land THERE. Only the navmesh (here in the core) can answer "is this
// a wall", which is why this can't live in the C# brain. Cheap-in-the-common-case (the first check
// usually passes); only a genuinely bad landing pays for the ring scan, and only on a port (rare).
// ============================================================
void AiBotAI::TeleportToWalkable(float x, float y, float z, float o, const char* tag)
{
    ...
    for (int r = 0; r < 5; ++r)
    {
        ...
        {
            float const ang = (float)i * (float)M_PI / 4.0f;
            float cx = x + cosf(ang) * radii[r];
            float cy = y + sinf(ang) * radii[r];
            float cz = z;
            ReGroundZ(cx, cy, cz, tag);
    ...
    sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL,
        "[AIBOT-PATH] %s: landing (%.1f,%.1f,%.1f) — NO walkable spot within 120yd, left as-is",
        me->GetName(), x, y, z);
}
